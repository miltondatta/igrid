...
route.post('/menu/entry', async (req, res) => {
    try {
        ...
        if (menus.length > 0) return res.status(400).json({
            msg: 'This Menu is already exist!',
            error: true
        });
        ...
        return res.status(200).json({msg: 'New Menu saved successfully.', success: true});
    } catch (err) {
        console.error(err.message);
...
route.delete('/menu/delete/:id', async (req, res) => {
    try {
        ...
        const status = await Menu.findOne({where: {id}});
        if (!status) return res.status(400).json({msg: 'This Menu didn\'t found!', error: true});
        ...
        return res.status(200).json({msg: 'One Menu deleted successfully!', success: true});
    } catch (err) {
        console.error(err.message);
        return res.status(500).json({msg: err.message});
    }
